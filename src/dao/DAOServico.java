...
public class DAOServico {
    ...
    public boolean alterar(Servico obj) {
        String sql = "update Servico set nome = ? where idServico = ?";
        try {
            PreparedStatement cod = Conexao.getPreparedStatement(sql);
            cod.setString(1, obj.getNome());
            cod.setInt(2, obj.getIdServico());
            if (cod.executeUpdate() > 0) {
                return true;
            } else {
                return false;
     ...
     */
    public ArrayList<Servico> localizarNome(String sql) {
        try {
